...
export const Provider = ({ children }) => {
            ...
            "categorys": [
              {
                ...
                "name": "Starters",
                "position": "0",
                "description": "Mouth watering starters",
                "path": 'starters',
                "menuItems": [
                  {
                    "id": "94298",
    ...
    const addToCart = (item) => {
        setCart((prevCart) => {
            ...
            // If the item doesn't exist, add it with quantity 1, excluding reviews
            const { reviews, ...itemWithoutReviews } = item; // Destructure to skip reviews
            return [...prevCart, { ...itemWithoutReviews, quantity: 1 }];
        });
    };


    const removeFromCart = (id) => {
        setCart(prevCart => prevCart.filter(item => item.id !== id)); // Remove dish from cart
    };
    ...
    const providedContext = useMemo(
        ...
        [dishes, cart], // Update when dishes or cart changes
    );
